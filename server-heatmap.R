...
updateSelectInput(
  ...
  selected = switch(inputDataReactive()$dataType,
    ...
    tryCatch(
      {
        if (!is.null(input$hmKeepBucket)) {
          ...
          output$heatmapDesign <- renderText({
            design
          })
          ...
          lapply(c("Both", "Up", "Down"), function(sig) {
            ...
            heatmap <- ComplexHeatmap::Heatmap(
              ...
              column_title = paste("Top", nrow(countsHeatmapSig), sig, "Regulated DEGs:\n", design)
            )
          ...
          observeEvent(
            ...
            ignoreNULL = F,
            {
              if (input$heatmapGoInput != "") {
                cat("Go:", input$heatmapGoInput)
                withProgress(message = "Making GO Heatmap. Can take a few minutes!", {
                  goToPlot <- gsub(" .*", "", input$heatmapGoInput)
                  goGenes <- seqAnnoHeatmap$gene_name[unique(as.numeric(unlist(sapply(seqAnnoHeatmap[, c("GO BP", "GO MF", "GO CC")], function(x) grep(goToPlot, x, ignore.case = TRUE)))))]
