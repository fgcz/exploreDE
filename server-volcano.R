...
if (inputDataReactive()$dataType == "proteomics") {
  updateSelectInput(session = session, inputId = "pTypeVolcano", selected = "Raw")
    ...
    tryCatch(
      {
        ...
        # Update the -log10p input to better reflect the values in the dataset
        updateNumericInput(
          session = session,
          inputId = "yLimVolcano",
        ...
        volcanoTable <- seqAnnoFilt[, c("gene_name", "log2Ratio", pTypeVolcano)]
        ...
        # Highlighted
        vc2 <- volcanoTable
        vc2 <- vc2[order(vc2$log10p, decreasing = TRUE), c("gene_name", "log2Ratio", pTypeVolcano, "log10p", "Status")]
        ...
        output$volcanoTable <- DT::renderDataTable({
          DT::datatable(
            ...
            rownames = FALSE,
            colnames = c("Gene Symbol", "Log2 Ratio", pTypeVolcano, paste("-log10", pTypeVolcano), "Significance")
          ) %>%
            DT::formatSignif(columns = "log10p", digits = 3) %>%
            DT::formatSignif(columns = pTypeVolcano, digits = 3) %>%
            ...
            DT::formatStyle(columns = pTypeVolcano, color = styleInterval(cuts = input$pThresholdVolcano, values = c("green", "black")), fontWeight = "bold") %>%
            DT::formatStyle(columns = "Status", color = styleEqual(levels = names(volcanoColours), values = as.character(volcanoColours)))
        })
...
observeEvent(
  ...
  },
  {
    ...
    if (input$volcanoShowGenes == TRUE & length(input$boxKeepBucketGenes) >= 1) {
      ...
      maplot <- maplot +
        ...
        geom_text_repel(
          ...
          aes(x = Log2_Mean, y = log2Ratio_full, label = genelabels),
          color = "black",
    ...
    output$MABrushTable <- renderTable({
      brushedPoints(volcanoTableFull[, c("gene_name", "log2Ratio_full", "Log2_Mean", "log10p_full", "Status")], input$MABrush)
    })
    ...
    if (input$volcanoShowGenes == TRUE & length(input$boxKeepBucketGenes) >= 1) {
      ...
      volcanoStatic <- volcanoStatic +
        geom_text_repel(
          data = volcanoHighlights,
          aes(x = log2Ratio, y = log10p, label = genelabels, label.size = NA, label.padding = 0.1, na.rm = TRUE, fill = alpha(c("white"),0.5)),
          color = "black",
          size = (input$textSizeVolcano / 3),
          max.overlaps = 5000, 
          fontface = "bold"
    ...
    output$volcanoBrushTable <- renderTable({
      brushedPoints(volcanoTableFull[, c("gene_name", "log10p_full", "log2Ratio_full", "log2Ratio", "log10p", "Status")], input$volcanoBrush)
    })
